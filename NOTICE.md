FloeBar
=======

FloeBar is an independent, modified version of Ice, a menu bar manager for
macOS. FloeBar is maintained by Evan Zhu and is not affiliated with, nor
endorsed by, the original Ice project or its author.

Original work
-------------
Ice
Copyright (C) 2024 Jordan Baird
https://github.com/jordanbaird/Ice
Licensed under the GNU General Public License v3.0.

Modifications
-------------
FloeBar
Copyright (C) 2026 Evan Zhu

FloeBar is based on Ice 0.12.0 and includes changes such as menu bar section
persistence, stability and memory optimizations, removal of the search feature,
English/Simplified Chinese localization, a redesigned application icon, and a
rebrand to FloeBar with its own bundle identifier. The upstream Sparkle update
feed has been removed so FloeBar cannot update itself from official Ice releases.

As required by the GNU General Public License v3.0, these modified files carry
prominent notices stating that they have been changed, and FloeBar is
distributed under the same GPL-3.0 license. See the LICENSE file for the full
license text.
