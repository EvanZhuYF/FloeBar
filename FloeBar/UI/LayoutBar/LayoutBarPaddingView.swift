...
final class LayoutBarPaddingView: NSView {
    ...
    override func performDragOperation(_ sender: NSDraggingInfo) -> Bool {
        ...
        if let index = arrangedViews.firstIndex(of: draggingSource) {
            if arrangedViews.count == 1 {
                ...
                let destination: MenuBarItemManager.MoveDestination? = switch section.name {
                case .visible:
                    items.first { $0.info == .hiddenControlItem }.map { .rightOfItem($0) }
                case .hidden:
                    items.first { $0.info == .hiddenControlItem }.map { .leftOfItem($0) }
                case .alwaysHidden:
                    items.first { $0.info == .alwaysHiddenControlItem }.map { .leftOfItem($0) }
                }
                if let destination {
    ...
    private func move(item: MenuBarItem, to destination: MenuBarItemManager.MoveDestination) {
        ...
        Task {
            ...
            do {
                try await appState.itemManager.moveByUser(item: item, to: destination, section: sectionName)
            } catch {
