...
final class MenuBarOverlayPanel: NSPanel {
    ...
    /// A Boolean value that indicates whether a periodic refresh can produce a visible result.
    var canPerformPeriodicRefresh: Bool {
    ...
    /// Performs a periodic fallback update using a shared window snapshot.
    func performPeriodicRefresh(with windows: [WindowInfo]) {
        guard
            canPerformPeriodicRefresh,
            let owningDisplay = validate(for: .updates, with: windows)
        else {
            return
        }
        performUpdates(
            for: [.applicationMenuFrame, .desktopWallpaper],
            windows: windows,
