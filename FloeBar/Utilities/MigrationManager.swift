...
extension MigrationManager {
    ...
    static func migrateAll(appState: AppState) {
        ...
        // Import an existing Ice installation's data before running the
        // version migrations, so imported settings flow through them too.
        migrateFromIce()
...
extension MigrationManager {
    ...
    private func migrateControlItems0_10_1() -> MigrationResult {
        ...
        if needsResetPreferredPositions {
            ...
            let alert = NSAlert()
            alert.messageText = String(localized: "Due to a bug in the 0.10.0 release, the data for FloeBar's menu bar items was corrupted and their positions had to be reset.")
